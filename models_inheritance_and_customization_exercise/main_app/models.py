...
class StudentIDField(models.PositiveIntegerField):
    def to_python(self, value):
        try:
            return int(value)  # int('string') will return an error.
        except ValueError:
            raise ValueError("Invalid input for student ID")

    def get_prep_value(self, value):  # called on level 'save' not 'clean'
        cleaned_value = self.to_python(value)  # to check if it is an INT or NOT.
...
class Room(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.clean()  # we call it because it cannot be called automatically
...
class BaseReservation(models.Model):
    ...
    def reservation_period(self):
        return (self.end_date - self.start_date).days  # taking the days
...
class RegularReservation(BaseReservation):
    def save(self, *args, **kwargs):
        super().clean()  # call the 'clean' method from BaseReservation
        super().save(*args, **kwargs)
...
class SpecialReservation(BaseReservation):
    ...
    def extend_reservation(self, days: int):
        ...
        self.save()  # if is_available we save it to our DB
