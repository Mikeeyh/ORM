...
def find_authors_nationalities():
    ...
    authors_names_with_nationalities = [f"{str(a)} is {a.nationality}" for a in authors_with_nationalities]
    return '\n'.join(authors_names_with_nationalities)
...
def filter_authors_by_birth_year(start_year, end_year):
    authors = Author.objects.filter(birth_date__year__range=(str(start_year), str(end_year))).order_by("-birth_date")
    result = []
