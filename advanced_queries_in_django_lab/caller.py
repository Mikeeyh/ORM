...
#     result = []
#     orders = Order.objects.prefetch_related('orderproduct_set__product__category').order_by('id')  # we do not have related name so we use the default name 'orderproduct__set. We take this and product__category too.
#     # We can remove prefetch_related logic and replace it with 'all()', this will work too.
#     for order in orders:
#         result.append(f"Order ID: {order.id}, Customer: {order.customer.username}")
#         for ordered_product in order.orderproduct_set.all():  # taking all products from the current customer's order
